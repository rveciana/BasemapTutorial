...
* When drawing the contour, the first band from each variable is taken, since in this casae, the model has several bands for longitudes, latitudes and temperature
...
* The limits in the map don't match those of the data to show that now, the shape of the data is rectangular, so the projection is properly configurated
